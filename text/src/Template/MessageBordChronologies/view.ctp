<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\MessageBordChronology $messageBordChronology
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Message Bord Chronology'), ['action' => 'edit', $messageBordChronology->message_bord_chronologies_id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Message Bord Chronology'), ['action' => 'delete', $messageBordChronology->message_bord_chronologies_id], ['confirm' => __('Are you sure you want to delete # {0}?', $messageBordChronology->message_bord_chronologies_id)]) ?> </li>
        <li><?= $this->Html->link(__('List Message Bord Chronologies'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Message Bord Chronology'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Message Bords'), ['controller' => 'MessageBords', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Message Bord'), ['controller' => 'MessageBords', 'action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="messageBordChronologies view large-9 medium-8 columns content">
    <h3><?= h($messageBordChronology->message_bord_chronologies_id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Message Bord') ?></th>
            <td><?= $messageBordChronology->has('message_bord') ? $this->Html->link($messageBordChronology->message_bord->title, ['controller' => 'MessageBords', 'action' => 'view', $messageBordChronology->message_bord->message_bords_id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $messageBordChronology->has('user') ? $this->Html->link($messageBordChronology->user->Array, ['controller' => 'Users', 'action' => 'view', $messageBordChronology->user->users_id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Message Bord Chronologies Id') ?></th>
            <td><?= $this->Number->format($messageBordChronology->message_bord_chronologies_id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($messageBordChronology->created) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Message') ?></h4>
        <?= $this->Text->autoParagraph(h($messageBordChronology->message)); ?>
    </div>
</div>
